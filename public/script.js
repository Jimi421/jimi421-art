...
async function loadGallery(group = 'all') {
  ...
  if (filtered.length === 0 && group !== 'all') {
    console.warn(`No images matched group "${group}", falling back to all.`);
    return loadGallery('all');
  }
  ...
  filtered.reverse().forEach(item => {
    ...
    card.onclick = () => {
      ...
      window.location.href = `/photo.html?group=${encodeURIComponent(groupName)}&filename=${encodeURIComponent(filename)}`;
    };
...
document.getElementById('uploadBtn').onclick = async () => {
  ...
  if (selectedGroup === '__new__') {
    const inputVal = document.getElementById('newGroupInput').value.trim();
    if (inputVal) {
